...
require([
	...
	var waterbodiesURL = "https://services1.arcgis.com/7iJyYTjCtKsZS1LR/arcgis/rest/services/Trout_Stocking/FeatureServer/0"; //Waterbodies layer
	var waterCentroidURL = "https://services1.arcgis.com/7iJyYTjCtKsZS1LR/arcgis/rest/services/Trout_Stocking/FeatureServer/1"; //Waterbodies layer
	...
	function makeGoogleQuery(results) {
		...
		console.log('SELECT A,E WHERE D = "' + myResult + '"')
		fishquery.setQuery('SELECT A,B,E WHERE D = "' + myResult + '"'); 
		orderOutForFish();
						  }
		  catch(err){
			  console.log("threw a rod!")
			  myResult = ""
		  }
	} 
    ...
    function handleQueryResponse(response) {
        ...
        data = response.getDataTable().getDistinctValues(2);
		...
		stockingDialog = new Dialog({
			title : dataWaterBody,
			content : "<div><b>Stocked date(s):</b> "+text2 + "<br />---<br /><b>Species:</b> " + text+"</div>",
			style : "width: 300px",
			duration: 1000
		});
		stockingDialog.show();
	...
	//################################################################################--------
	var waterbodySymbol = new SimpleFillSymbol().setColor(new Color([230, 0, 0, 1]));
	waterbodySymbol.outline.setStyle(SimpleLineSymbol.STYLE_SOLID);
