...
require([
	...
	var riversURL = "http://209.80.128.244/arcgis/rest/services/DFG/TSA_Test/MapServer/0"; //Openspace layer
	var pondsURL = "http://209.80.128.244/arcgis/rest/services/DFG/TSA_Test/MapServer/1"; //Openspace layer
	var lite_URL = "http://maps.env.state.ma.us/dev/dfg/lis_lite/index.html?EXTENT=" // permalink url part
	...
	var fishquery = new google.visualization.Query('https://docs.google.com/spreadsheets/d/171W-DbZzBeH71C0C35JNyYWpqKVjUXL1ynF2O-xvIl8/gviz/tq?tqx=out:json&gid=1168903987');
	
	var stockedQuery = new google.visualization.Query('https://docs.google.com/spreadsheets/d/171W-DbZzBeH71C0C35JNyYWpqKVjUXL1ynF2O-xvIl8/gviz/tq?tqx=out:json&gid=1168903987');
	stockedQuery.setQuery('SELECT H,A');
	stockedQuery.send(createDefQuery)
	
	function createDefQuery(response){
		data = response.getDataTable().getDistinctValues(0);
		data = data.slice(1,data.length)
		defQueryString = "PALIS_ID IN (" + data + ")"
		    ponds.setDefinitionExpression(defQueryString)
		//alert(defQueryString)
	}
		...
		map.on("click", function(evt) {
		 ...
		 var query = new Query();
		  ...
		  ponds.queryFeatures(query, makeGoogleQuery);
        });
	
	function makeGoogleQuery(results) {
		myResult = results.features[0].attributes.PALIS_ID
		fishquery.setQuery('SELECT A,Q WHERE H CONTAINS ' + myResult); //fishquery.setQuery('SELECT A,Q WHERE I CONTAINS "35017-343"');
		orderOutForFish();
	}
    ...
    function handleQueryResponse(response) {
			console.log("queries finished: ", response);
			...
			data = response.getDataTable().getDistinctValues(1);
			...
			//ponds.setInfoTemplate(stockingInfo);
			if (text.length != 0) {
			//alert(text2 + " --- " + text);
			}
			text = "";
	...
	//################################################################################--------
	var pondSymbol = new SimpleFillSymbol().setColor(new Color([230, 0, 0, .80])); //greenish
	pondSymbol.outline.setStyle(SimpleLineSymbol.STYLE_SOLID);
	var pondRenderer = new SimpleRenderer(pondSymbol)	
	
	var rivers = new FeatureLayer(riversURL, {
			//infoTemplate : new InfoTemplate("<b>${BAS_ID}</b>", "Saris ID: ${SARIS_ID}"),
			//infoTemplate : new InfoTemplate(text2),
			mode : 1,
			id : "rivers",
			opacity : 0.5,
			outFields : ["SARIS_ID","BAS_ID"],
		    minScale : 200000
		});
	var ponds = new FeatureLayer(pondsURL, {
			//infoTemplate : new InfoTemplate("<b>${NAME}</b>", "NO STOCKING OCCURED"),
			mode : 0, // 1 =.MODE_ONDEMAND,
			id : "ponds",
			opacity : 0.5,
			outFields : ["PALIS_ID","NAME"],
			infoTemplate: popupTemplate
			//minScale : 200000
		});
    ponds.setRenderer(pondRenderer);
    //ponds.setDefinitionExpression(defQueryString)
	...
	townsurvey.setRenderer(townrenderer);	//https://maps.env.state.ma.us/dev/dfg/lis_lite/index.html?EXTENT=PROJ_ID1=%27DFW1323%27%20OR%20PROJ_ID1=%27DFW1039%27%20OR%20PROJ_ID1=%27DFW0505%27
	//SAMPLE QUERY URL	
	...
	dojo.addOnLoad(function() {//wait for the DOM to load otherwise... map undefined
		var URLparam = getUrlParam("EXTENT").slice(0, 4)
			if (URLparam === "PROJ") {
				placeQueryAndZoom(getUrlParam("EXTENT"), OpenSpaceURL); //Read & Use LIS Name Site Parameter
			}else
			{
		...
		//Zoom and set buttons from URL parameters
//http://146.243.225.201:8080/lis_lite/index.html?DEFQUERY=OWNER_ABRV=%27DFG%27%20OR%20OWNER_ABRV=%27DCRS/DFG%27&EXTENT=PROJ_ID1=%27DFW0600%27
//Could be for a show just this property, or just one agency's land
	if (getUrlParam("DEFQUERY") !== null) { //if there is no parameter
	dojo.addOnLoad(function() {rivers.setDefinitionExpression(getUrlParam("DEFQUERY"));});
	}
rivers.setDefinitionExpression("SARIS_ID>0")
...
//#########Print Button, Permalink Button, Permalink Dialog
	...
	map.addLayers([rivers, ponds, townsurvey]);
...
*/
	...
	function handleError(err) {
		console.log("Unable to get webmap from DFW's Server: ", err);
	}
